...
$sqlWhere = '';
if(!empty($s_title)){
	$sqlWhere.= "		and category_title like '%".$s_title."%' ";
}

//게시글 리스트
$sql = "
	select count(category_seq) as bbs_count
...
if($totalCount > 0){
	...
	$bbsList = array();
	$sql = "
		select category_seq, category_title, category_regdt
			from at_category_info
		where category_status = 1
	";
	$sql.= $sqlWhere;
	$sql.= "		group by category_seq ";
	$sql.= "		order by category_seq desc ";
